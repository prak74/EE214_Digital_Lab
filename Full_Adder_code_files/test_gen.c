//C code for TRACEFILE.txt generation for FULL-ADDER
#include <stdio.h> 
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
{ 
   ...
   optr = fopen(output_file, "w");
	...
	{
		input = i;
		//seperate the inputs(binary form)
		a = i / 4;
		b = (i % 4) / 2;
		cin = i % 2;

		//convert the binary inputs to decimal(not needed for 1 bit inputs)
		a = bin_to_dec(a);
		b = bin_to_dec(b);
		cin = bin_to_dec(cin);
